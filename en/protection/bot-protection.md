...
| Scenario | Path | Match | Mode | Recommendation |
| --- | --- | --- | --- | --- |
| Admin entry | `/admin` | `prefix` | `js-challenge` | Start with `log-only`, then block |
| Login page | `/login` | `exact` | behavior scoring + JS challenge | Observe from threshold 60 |
| High-risk transaction | `/checkout` | `prefix` | captcha | Enable device-signal binding |
| SEO page | `/` | `prefix` | JS challenge | Enable search-engine bypass only when needed |

## Match Scope

Bot rules support `exact` and `prefix` path matching. An empty method list means all methods. `prefix` uses path segment boundaries to avoid matching `/admin2` from `/admin`.
