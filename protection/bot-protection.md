...
| 场景 | 路径 | 匹配 | 模式 | 建议 |
| --- | --- | --- | --- | --- |
| 后台入口 | `/admin` | `prefix` | `js-challenge` | 先 `log-only`，再阻断 |
| 登录页 | `/login` | `exact` | 行为评分 + JS challenge | 阈值从 60 观察 |
| 高风险交易 | `/checkout` | `prefix` | captcha | 开启设备信号绑定 |
| SEO 页面 | `/` | `prefix` | JS challenge | 只在明确需要时开启搜索引擎绕过 |

## 匹配范围

Bot 规则路径支持 `exact` 和 `prefix`。方法为空表示全部方法。`prefix` 按路径段边界处理，避免 `/admin` 误命中 `/admin2`。
